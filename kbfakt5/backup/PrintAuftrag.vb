...
Public Class PrintAuftrag
    ...
    'Dim pSize As New PaperSize("A4lang", pWidth, pHeight)
    ...
    Private Sub PrintAuftrag_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        For i = 0 To PrintDocument1.PrinterSettings.PaperSizes.Count - 1
            ...
            If pkSize.RawKind = m_iPaperKind Then
                ...
                PrintDocument1.DefaultPageSettings.PaperSize.RawKind = PaperKind.GermanStandardFanfold
                Debug.WriteLine("Found and assigned 'Endlospapier 8.5 x 12 Zoll'")
                foundIt = True
                'Exit For
            End If
        Next
        If Not foundIt Then Debug.WriteLine("WARNING: 'Endlospapier 8.5 x 12 Zoll' not found")
    ...
    Private Sub FillKopfzeilen(ByVal AuftragsNummer As String)
        ...
        SkipLines(4)
        KopfZeilen.Add("^E" + DruckTyp + " " + (AuftragsNummer) + " vom " + txtDatum)
        ...
        KopfZeilen.Add(("Datum:").PadLeft(LeftOffset) + (txtWerkdatum).PadLeft(12))
    ...
    Private Sub PrintForm(ByVal AuftragsNummern As Collection)
        ...
        Dim FremdteileSumme As Double = 0
        ...
        For AuftragID = 1 To AuftragsNummern.Count
            ...
            ZwischenSumme = 0 : SchmiermittelSumme = 0 : FremdteileSumme = 0
            LineCount = 0
            PageCurrent = 1
            pCount = 0
            Try
            ...
            For i = 0 To numberOfItems - 1
                ...
                With dbset.Tables(0).Rows(i)
                    ...
                    ZwischenSumme += G_Preis
                    If CInt(.Item("arttyp")) = 1 Then SchmiermittelSumme += G_Preis
                    If CInt(.Item("arttyp")) = 3 Then FremdteileSumme += G_Preis 'change in 2.0.0.3
                ...
                End With
            Next i
            Dim MwSt As Double = ZwischenSumme / 100 * CDouble(txtMwStSatz)
            Dim Brutto As Double = ZwischenSumme + MwSt
            Dim FremdteileMwSt As Double = FremdteileSumme / 100 * CDouble(txtMwStSatz)
            ...
            If PreiseAnzeigen Then
                ...
                LineCount += 1
                'Fremdteile
                If (FremdteileSumme > 0) Then
                    FStream.Write("Fremdteile: ".PadLeft(19) + Format(FremdteileSumme, "0.00").PadLeft(8))
                Else
                    FStream.Write(" ".PadLeft(19) + Format(" ").PadLeft(8))
                End If
                'MwSt Betrag
                ...
                LineCount += 1
                If (FremdteileSumme > 0) Then
                    FStream.Write("Fremdteile MwSt: ".PadLeft(19) + Format(FremdteileMwSt, "0.00").PadLeft(8))
                Else
                    FStream.Write(" ".PadLeft(19) + Format(" ").PadLeft(8))
                End If
                'Gesamtbetrag
    ...
    Private Sub btnPrintRAW_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintRAW.Click
        With PrintDialog1
            .Document = PrintDocument1
            .Document.DocumentName = DruckTyp
            .AllowCurrentPage = False
